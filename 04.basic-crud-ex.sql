-- ex.01 Find All Information About Departments 
SELECT * FROM `departments`
ORDER BY `department_id` ASC;

-- ex.02 Find all Department Names 
...
-- ex.05 Find Email Address of Each Employee 
SELECT concat(`first_name`, '.', `last_name`, '@softuni.bg') 
AS `full_email_addres`
FROM `employees`;

-- ex.06 Find All Different Employee's Salaries 
SELECT DISTINCT `salary` FROM `employees`;

-- ex.07 Find all Information About Employees 
SELECT * FROM `employees`
WHERE `job_title`='Sales Representative';

-- ex.08 Find Names of All Employees by salary in Range 
SELECT `first_name`, `last_name`, `job_title` 
FROM `employees`
WHERE `salary` BETWEEN 20000 AND 30000
ORDER BY `employee_id`;

-- ex.09 Find Names of All Employees  
